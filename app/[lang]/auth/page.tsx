        ...
        {isLoading ? (
          ...
          <Card className="max-w-80">
            {isEmailSent ? (
              <>
                <Input
                  label={`${dictionary.auth.enterCode} ${email}`}
                  type="text"
                  value={code}
                  onChange={setCode}
                />
                {code.length === 6 ? (
                  <Box onClick={handleSendCode}>{dictionary.auth.continue}</Box>
                ) : null}
              </>
            ) : (
              <>
                <Input
                  label={dictionary.auth.registerOrLogin}
                  type="email"
                  value={email}
                  onChange={setEmail}
                />
                {/^[\w.-]+@(?:[\w-]+\.)+[\w-]{2,4}$/u.test(email) ? (
                  <Box onClick={handleSendEmail}>
